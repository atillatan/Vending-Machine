...
dotnet publish src/Machine.CLI -r linux-x64 --self-contained -c $BUILD_CONFIGURATION --output "$PUBLISH_PATH/linux-cli"
xcopy "config" "$PUBLISH_PATH/linux-cli/config"  %cpycmd%

echo %BUILD_VERSION% > "$PUBLISH_PATH/linux-api/version.json"
...
dotnet publish src/Machine.API -r linux-x64 --self-contained -c $BUILD_CONFIGURATION --output "$PUBLISH_PATH/linux-api"

cp -R ../config $PUBLISH_PATH/linux-cli
cp -R ../config $PUBLISH_PATH/linux-api

echo $BUILD_VERSION > version.json
mv version.json $PUBLISH_PATH/linux-cli
echo $BUILD_VERSION > version.json
mv version.json $PUBLISH_PATH/linux-api
