...
dotnet publish src/Machine.CLI -r linux-x64 --self-contained -c $BUILD_CONFIGURATION --output "build/linux-cli"
cp -R ../config macos-cli

echo %BUILD_VERSION% > "build/linux-api/version.json"
...
mv version.json linux-cli
echo $BUILD_VERSION > version.json
mv version.json linux-api
